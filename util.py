...
class Rider:
    ...
    # Calculate the cost of a delivery person for a given distance
    # = fixed cost per delivery person + variable cost calculated by distance traveled
...
# calculate the total volume of the orders
# shop_seq is the pickup list of orders
# Note: shop_seq is the same as list in order id
def get_total_volume(all_orders, shop_seq):
    return sum(all_orders[k].volume for k in shop_seq)

# Calculate total distance when picking up in order shop_seq and delivering in order dlv_seq
# Note: shop_seq and dlv_seq must have the same order ids. That is, set(shop_seq) == seq(dlv_seq). (The order of the order IDs can be reversed)
...
# Return pickup and delivery times when picking up in order of shop_seq and delivering in order of dlv_seq
# Note: shop_seq and dlv_seq must have the same order id. i.e. set(shop_seq) == seq(dlv_seq). (The order of order IDs can be reversed)
...
# test if bundled order constraints are satisfied when picked up in order shop_seq and delivered by delivery rider in order dlv_seq
# return 0 if all constraints are satisfied
# return -1 if capacity constraints are not satisfied
# Return -2 if time constraints are not satisfied
# Note: shop_seq and dlv_seq must have the same order id. That is, set(shop_seq) == seq(dlv_seq). (The order of the order IDs can be reversed)
...
# test if two bundles can be bundled while satisfying constraints
# check all possible pickup/delivery combinations for the combined bundle's shipping route
# if it is possible to combine the two bundles, return the new combined bundle
# If it's not possible to combine, return None
# Note: this only tests for couriers that deliver the two given bundles (it may be possible to bundle couriers that are not used for a given bundle!)
# Note: if it is possible to combine multiple couriers, return the first available courier (without considering cost)
...
# test if it is possible to change the delivery personnel for a given bundle
# Note: double-check the visit order combinations as the original bindle's visit order may not be optimal
...
# Returns the courier with the lower *variable* cost among the remaining couriers
# return None if there is no cheaper courier
...
# return two randomly from the given bundle list (without duplicates)
def select_two_bundles(all_bundles):
    bundle1, bundle2 = random.sample(all_bundles, 2)
    return bundle1, bundle2

# Calculate the average cost (objective function)
# = total cost / number of orders
def get_avg_cost(all_orders, all_bundles):
    return sum([bundle.cost for bundle in all_bundles]) / len(all_orders)

# return from a given bundle list in solution format for submission
...
# Test the feasibility of a given solution
# Note: solution is a list of [delivery type, pickup order, delivery order].
# If the solution is feasible, the returning dict contains additional information such as the average cost.
# If solution is infeasible, the reason is returned as 'infeasibility' item (key)
...
# visualize the path of a given solution
...
# visualize bundled delivery visitation times for a given solution
